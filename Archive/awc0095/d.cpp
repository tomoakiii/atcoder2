...
int main(){
    ...
    rep(i,N) {
        ll l,r,v; cin>>l>>r>>v;
//        A[i]=make_tuple(l,r,v);
        A[i]=make_tuple(r,l,v);
    }
    sort(A.begin(), A.end());
    ll ful=1ll<<N;
    ll ans=INF;
    ...
    rep(i,K) select[i]=false;

    do{
        SegTree<ll> ST(vector<ll>(101,0));
        rep(i,N){
            if(!select[i]) continue;
//            auto [l,r,v]=A[i];
            auto [r,l,v]=A[i];

            auto [mx,j] = ST.MaxElement(0,0,l);
            if(ST.GetVal(r) < mx+v) ST.SetVal(r, mx+v);
        }
        auto [mx,j] = ST.MaxElement(0,0,100);
        chmin(ans, mx);
