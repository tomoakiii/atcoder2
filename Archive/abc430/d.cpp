...
int main(){
    ...
    vector<ll>X(N+1);
    rep(i,N)cin>>X[i+1];
    set<pair<ll,int>> st0, st1;
    st0.insert({0,0}); st1.insert({0,0});
    vector<ll> dist(N+1);
    SegTree<ll> ST(dist);
    dist[0] = INF;
    for(int i=1; i<=N; i++) {
        ll x = X[i];
        auto s0 = st0.lower_bound({x,i});
        auto s1 = st1.lower_bound({-x,i});
        ll d = INF;
        if(s0 != st0.end()) {
            ll c = abs(s0->first - x);
            chmin(dist[s0->second], c);            
            chmin(d,c);
            ST.SetVal(s0->second, dist[s0->second]);
        }
        if(s1 != st1.end()) {
            ll c = abs(s1->first + x);
            chmin(dist[s1->second], c);
            chmin(d,c);
            ST.SetVal(s1->second, dist[s1->second]);
        }
        dist[i] = d;
        ST.SetVal(i, dist[i]);
        cout<<ST.GetSum(0,0,N)<<endl;
        st0.insert({x,i});
        st1.insert({-x,i});
    }
