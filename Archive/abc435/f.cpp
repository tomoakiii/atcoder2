...
int main(){
    ...
    rep(i,N) cin>>A[i];
    SegTree<ll> ST(A);
    vector<pair<int,int>> TR(N, {-1,-1});
    auto func = [&](auto func, int parent=-1, int l, int r)->void{
        if(l <= parent-1){
            auto [x, i] = ST.MaxElement(0, l, parent-1);
            if(parent != -1) TR[parent].first = i;
            if(l<= parent-1) func(func, i, l, parent-1);
        }
        if(parent+1 <= r){
            auto [x, i] = ST.MaxElement(0, parent+1, r );
            if(parent != -1) TR[parent].second = i;
            if(parent+1 <= r) func(func, i, parent+1, r);
        }
    };
    auto [x, i] = ST.MaxElement(0, 0, N-1);
    func(func, i, 0, N-1);
    vector<ll> dpt(N, -1);
    dpt[i] = 0;
