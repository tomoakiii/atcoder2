...
int main(){
    ...
    lazy_segtree<S, op, e, F, mapping, composition, id> LS(vector<S>(N, 0));
    SegTree<ll> Seg(A);
    SegTree<int> EmptySeg(vector<int>(N,1));
    int Q; cin>>Q;    
    rep(q,Q) {
        int l, r; ll k;
        cin>>l>>r>>k;
        l--; r--;
        LS.apply(l,r+1,k);
        ll ans = k * EmptySeg.GetSum(0, l,r);
        while(true){
            auto v = Seg.MinElement(0, l, r);            
            ll x = LS.prod(v.i,v.i+1);
            if(v.v < x) {
                EmptySeg.SetVal(v.i, 0);
                Seg.SetVal(v.i, INFi);
                ans -= (x - v.v);
            } else {
                break;
