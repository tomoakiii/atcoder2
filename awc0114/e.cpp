...
int main(){
    ...
    ll mr=-INF;
    rep(i,M){
        if(chmax(mr, LR[i].second)) {
            L.push_back(LR[i]);
        }
    }
    ll sz = L.size();
    SegTree<ll> ST1(vector<ll>(N,0));
    SegTree<ll> ST2(Vn);
    ...
    rep(i,sz){
        ...
        auto [l,r]=L[i];
        if(l==0) {
            chmax(dp[r], mx + V[r+1]-V[l]);
        } else {
            auto [mx,v] = ST1.MaxElement(0,l-1);
            chmax(dp[r], mx + V[r+1]-V[l]);
            auto [mx2, v2] = ST2.MaxElement(l,r);
            chmax(dp[r], mx2 + V[r+1]-V[v2]);
            chmax(dp[r], Vn[r] + dp[r]);
        }
        swap(dp,dp2);
    }
    ll ans=0;
    rep(k,K+1)chmax(ans,dp[k].first);
    cout << ans << endl;
    return 0;
}